...
tape('dirty#drop', t => {
  modesAndComparators.forEach(({mode, comparator}) => {
    ...
    t.ok(comparator('sepalLength')(modifiedRow), `${mode} should execute drop correctly`);
  });
...
tape('dirty#dropRow', t => {
  ...
  dropRow(data, 17);
  t.ok(data.every(row => !shallowDeepEqual(row, targetRow)), 'target row is excluded');
...
tape('dirty#dropColumm', t => {
  modesAndComparators.forEach(({mode, comparator}) => {
    ...
    t.ok(data.every(comparator('sepalLength')), `${mode} should be dropColum correctly`);
  });
  t.end();
});
...
tape('dirty#recast', t => {
  ...
  Object.keys(IRIS[0]).forEach(key => recastModes.forEach(({mode, expectedType}) => {
    const data = clone(IRIS);
    recast(data, key, mode);
    t.deepEqual(
      collectTypes(data, key), new Set([expectedType]),
...
tape('dirty#duplicate', t => {
  ...
  const targetCount = data.reduce((acc, row) =>
    acc + (shallowDeepEqual(row, targetRow) ? 1 : 0), 0);
  t.equal(targetCount, 10 + 1,
